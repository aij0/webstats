...
class PostgresDatabase:
    ...
    def setup_database_in_postgres(self, db_connection, database):
        try:
            storing_query(db_connection, self.create_postgres_servers_table)
            logging.debug("Query %s succeeded",
                          self.create_postgres_servers_table)
        ...
        try:
            ...
            storing_query(db_connection,
                          self.create_postgres_stats_table)
            logging.debug("Query %s succeeded",
                          self.create_postgres_stats_table)
        except Exception as err:
            logging.error("Query failed with %s", err)
    ...
    def setup_database(self, database):
        ...
        self.setup_database_in_postgres(db_connection, database)
